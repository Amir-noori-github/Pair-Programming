...
beforeAll(async () => {
  ...
  // Create User A
  const resultA = await api.post("/api/users/signup").send({
    name: "John Doe",
    email: "john@example.com",
    password: "4wa94=Vr++",
    phone_number: "1234567890",
  ...
  tokenUserA = resultA.body.token;

  // Create User B
  const resultB = await api.post("/api/users/signup").send({
    name: "Jane Smith",
...
describe("Given there are initially some tours saved", () => {
  ...
  // 🔒 New tests for cross-user access
  it("should NOT allow User B to access User A’s tour", async () => {
    const tour = await Tour.findOne();
    await api
      .get("/api/tours/" + tour._id)
      .set("Authorization", "bearer " + tokenUserB)
      .expect(404); // or 401 depending on your controller logic
  });

  it("should NOT allow User B to delete User A’s tour", async () => {
    const tour = await Tour.findOne();
    await api
      .delete("/api/tours/" + tour._id)
      .set("Authorization", "bearer " + tokenUserB)
